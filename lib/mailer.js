...
function sendVerificationCode(toEmail, code) {
  ...
  if (!transporter) {
    return Promise.resolve({ sent: false, reason: 'no-smtp', code: code });
  }
  return transporter.sendMail({ from: FROM, to: toEmail, subject: subject, text: text })
    .then(function (info) { return { sent: true, info: info }; })
    .catch(function (err) { return { sent: false, reason: String(err), code: code }; });
}
